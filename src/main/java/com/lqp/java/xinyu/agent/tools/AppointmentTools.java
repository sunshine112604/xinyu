...
public class AppointmentTools {
   ...
   @Tool(name = "预约挂号", value = "根据参数，先执行工具方法queryDepartment查询是否可预约，并直接给用户回答是否可预约，" +
    ...
    @Tool(name = "查询是否有号源",value = "根据科室名称，时期，时间和医生查询是否有号源，并返回给用户")
    public boolean queryDepartment(
            @P(value = "科室名称") String name,
            @P(value = "日期")String date,
            @P(value = "时间，可选值：上午、下午")String time,
            @P(value = "医生名称",required = false)String doctorName//医生是可选的参数
    ){
        System.out.println("查询是否有号源");
        System.out.println("科室名称"+name);
        System.out.println("日期"+ date);
        System.out.println("时间"+time);
        System.out.println("医生名称"+doctorName);//要直接调用业务系统中的方法

        //TODO维护医生的排班信息：
        //如果没有指定医生的名字，则根据其他条件查询是否可以有预约的医生（有返回true，否则返回false）；

        //如果指定了医生的名字，则判断医生是否有排班（没有排班返回false）
        //如果有排班，则判断医生排班时间是否已经约满（约满返回false，有空闲时间返回true）
